...
void DeathPlayer::_bind_methods() {
    ...
    // register property group for editor organization
...
void DeathPlayer::_ready() {
    // Load obstacle scenes used by the Death player when spawning traps.
...
void DeathPlayer::_process(double delta) {
    // Update the spawn cooldown timer each frame.
    if (cooldown_timer > 0) {
    ...
    // Only process if this is the Death player
            ...
            void DeathPlayer::spawn_obstacle(Vector2 position, int type) {
                ...
                if (scene.is_valid()) {
                    ...
                    if (obstacle) {
                        // Add obstacle to the scene as a sibling of this node's parent
                        get_parent()->add_child(obstacle);

                        // Set obstacle position if it's a Node2D
                        Node2D *obstacle_2d = Object::cast_to<Node2D>(obstacle);
                        if (obstacle_2d) {
                            obstacle_2d->set_position(position);
                        }

                        // Track spawn counts and reset cooldown
                        spawned_count++;
